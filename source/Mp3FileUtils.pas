...

{
 Extract and set several information in mp3-Files.
  ...
  =======================
    ...
    - Unterstьtzung von Unsynchronisation
    - Unterstьtzung von GroupID und DataLength-Flags in Frame-Headern
    - Bei Compression und/oder Encryption wird das Auslesen abgebrochen
    - Unterstьtzung von URLs
    - Unterstьtzung von Bewertungen
    - Fehler in der Behandlung bei "Beschreibungen" mit Unicode - das fьhrte u.a. dazu,
      dass viele Cover in Dateien von jamendo.com nicht angezeigt wurden.

  Februar 2008: v0.3a -> 0.3b
  ==========================
    - Funktion GetTrackFromV2TrackString hinzugefьgt
    - Bug in der Funktion GetPaddingSize behoben

  Juni 2007: v0.3 -> 0.3a
  ==========================
    - Bei den Gettern des ID3v1-Tags wurden hдufig Leerstellen und/oder Nullbytes mit ьbergeben, was ein Trim() auЯerhalb
      der Klasse nцtig machte. Das wurde korrigiert - das trim() wird jetzt hier intern gemacht.
    - Schцnheitsfehler bei der Benennung beseitigt: TPictureFrameDes()ription heiЯt jetzt TPictureFrameDes(c)ription


  Februar 2007: v0.2a -> 0.3
  ==========================
    - INT/WE- Versionen ьber Compiler-Schalter vereint.
      Siehe dazu das Ende dieses einleitenden Kommentars
    - Fehler entfernt, die bei einer Verkleinerung des ID3v2-Tags unter gewissen Umstдnden zu unschцnen
      Effekten bei den getaggten mp3s fьhrte - die letzten Frames/Sekunden des Liedes wurden dann doppelt
      abgespielt.
    - intelligenteres Padding-System (abhдngig von der ClustergrцЯe)
      fьr den ID3v1Tag werden 128 Byte im Cluster freigehalten (falls er nicht existiert), so
      dass ein nachtrдgliches Einfьgen keinen Zusatzplatz benцtigt.


  ========================================================================================================
  September 2006: v0.2 -> 0.2a (beide Versionen)
  ==============================================
    - katastrophalen Bug behoben, der ungьltige ID3v2-TextFrames erzeugt.
  ...
  =========================================
    ...
    Kleinere Bugs:
    ==============
    - Der ID3v1-Tag wurde vor dem Lesen nicht gelцscht, so dass u.U noch alte Informationen ьbrigblieben
    - Unter gewissen Umstдnden wurden Lyrics und Comments bei ID3v2 nicht richtig ausgelesen.
      Das lag aber an fehlerhaften Language-Informationen, die jetzt ausgebьgelt werden.
    - Fehler beim Lesen einer Variante von Unicode behoben (Stichwort: Byte-Order)
    - Finalize-Abschnitt (wieder) hinzugefьgt. Der ist zwischendurch mal irgendwo verlorengegangen.

    Updates/Дnderungen:
    ===================
       Klasse TID3v1Tag:
       -----------------
       - in der 'International'-Version werden alle Textinformationen als WideString zurьckgeliefert
       - beim Lesen findet ggf. eine Konvertierung statt, die vom CharCode abhдngt.
         ...
         Das funktioniert natьrlich nur mit einer gewissen Fehlerquote. Mehr dazu in der Datei 'Unicode.txt'.
       - beim Schreiben wird ebenfalls dieser Zeichensatz verwendet und der WideString entsprechend konvertiert
       - Flag 'AcceptAllEncodings' hinzugefьgt. Ist dieser 'False', findet keine Konvertierung statt
         (weder beim schreiben, noch beim lesen).
         Was das fьr einen Effekt auf Systemen auЯerhalb Westeuropas hat, kann ich nicht genau sagen.

       Klasse TID3v2Tag:
       -----------------
       - Sдmtliche TextInformationen werden jetzt als WideString geliefert.
       - Textinformationen werden automatisch im Unicode-Format gespeichert, falls dies nцtig ist.
       - Flag 'AlwaysWriteUnicode' hinzugefьgt. Ist dies gesetzt, wird immer im Unicode-Format gespeichert,
         auch wenn das nicht nцtig ist (d.h. wenn nur "Standard"-Buchstaben verwendet werden)
       - beim Lesen findet ggf. eine Konvertierung statt, die vom CharCode abhдngt.
         Mit Hilfe der Funktion GetCharCode aus der Unit U_CharCode (mitgeliefert) kann
         der beim Taggen verwendete Zeichensatz anhand des Dateinamens bestimmt werden.
         ...
         Kram mit der Konvertierung ьberhaupt implementieren musste.
         Das funktioniert natьrlich nur mit einer gewissen Fehlerquote. Mehr dazu in der Datei 'Unicode.txt'.
       - Flag 'AcceptAllEncodings' hinzugefьgt. Ist dieser 'False', findet keine Konvertierung statt
...
uses
  SysUtils, Classes, Windows, Contnrs, dialogs, FileUtil, U_CharCode
...
//--------------------------------------------------------------------
  ...
  public
    ...
    function GetPicture(stream: TStream; Description: UnicodeString): AnsiString; // Rьckgabe: Mime-Type
...
begin
  result := FileExistsUTF8(aString); { *Преобразовано из FileExists* }
end;
...
begin
  ...
  repeat
    ...
    inc(i);
  until not FileExistsUTF8(result); { *Преобразовано из FileExists* }
end;
    ...
    if RawHeader.ID = 'ID3' then
      ...
      begin
        ...
        begin
            ...
            // thanks to Jьrgen vom Projekt inEx information explorer
        end;
      end
      else
  ...
  try
    ...
    try
      ...
      begin
          ...
          // delete cache
          DeleteFileUTF8(PChar(TmpName)); { *Преобразовано из DeleteFile* }
      ...
      ID3v2Stream.Free;
      // delete cache
      DeleteFileUTF8(PChar(FrameName)); { *Преобразовано из DeleteFile* }
    end;
  except
...
initialization
  ...
  LanguageCodes.Add('vol');  LanguageNames.Add('Volapьk');
